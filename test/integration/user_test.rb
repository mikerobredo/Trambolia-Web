...
class UserTest < ActionDispatch::IntegrationTest
  ...
  test "user_account" do
    ...
    user_account = george.creates_user_account(:user => { :name => 'Pepe Malia', :login => 'pepe',
                                               :email => 'pepe@emporium.com', :password => 'gold',
                                               :password_confirmation => 'gold' })
    george.shows_user_account user_account
    george.edits_user_account(user_account, :user => { :name => 'Jeremy Keith', :login => 'jeremy',
                                            :email => 'jeremy@emporium.com', :password => 'silver',
  ...
  module BrowsingTestDSL
    ...
    def creates_user_account(parameters)
      ...
      assert_equal user_name, User.find_by_login(parameters[:user][:login]).name
      assert_equal flash[:notice], "La cuenta #{user_name} fue creada con éxito. Usuario autenticado."
      assert_select 'div#notice', "La cuenta #{user_name} fue creada con éxito. Usuario autenticado."
    ...
    def edits_user_account(user_account, parameters)
      ...
      user_name = parameters[:user][:name]
      assert_equal user_name, User.find_by_login(parameters[:user][:login]).name
      assert_equal flash[:notice], "La cuenta #{user_name} se actualizó correctamente."
      assert_select 'div#notice', "La cuenta #{user_name} se actualizó correctamente."
    end
  end

class UserController < ApplicationController
  ...
  def new
    ...
    if current_user
      flash[:notice] = 'Sólo una cuenta puede ser creada.'
  ...
  def create
    ...
    if @user.save # the new user has been logged in automatically
      flash[:notice] = "La cuenta #{@user.name} fue creada con éxito. Usuario autenticado."
  ...
  def update
    ...
    if @user.update_attributes(user_params)
      flash[:notice] = "La cuenta #{@user.name} se actualizó correctamente."
      redirect_to :action => 'show'
    else
      @page_title = 'Editar cuenta'
      render :action => 'edit'
